...
x_model = model.simulate(x0=np.zeros((4, )), t=t, u=u_MISO.T)
...
plt.plot(data.t[:-1], x_model[:, 0], '--', label='SINDy')
plt.ylabel(r'$\dot{h}$')
plt.legend()
plt.tick_params('x', labelbottom=False)
# alpha
...
plt.plot(data.t[:-1], x_model[:, 1], '--', label='SINDy')
plt.ylabel(r'$\alpha$')
plt.legend()
plt.tick_params('x', labelbottom=False)
# alpha'
...
plt.plot(data.t[:-1], x_model[:, 2], '--', label='SINDy')
plt.xlabel('t [-]')
...
C_L_SINDy = (np.atleast_2d(x_model[:, 3]).T + Du.T[:-1])
...
plt.subplot(4, 1, 3)
C_L_x_SINDy = np.atleast_2d(x_model[:, 3]).T
